...
public class BoardGameFragment extends Fragment implements Response.Listener<String>, View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        etSearch.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                String where = " WHERE BoardGameName LIKE '%" + etSearch.getText().toString() + "%'";
                showBoardGame(where);
            }
    ...
    private void showBoardGame(String s) {
        try {
            query = URLEncoder.encode("SELECT * FROM boardgame" + s + " ORDER BY BoardGameID DESC", "UTF-8");
        }
        catch (Exception e) {
    ...
    public class SimpleStringRecyclerViewAdapter extends RecyclerView.Adapter<SimpleStringRecyclerViewAdapter.ViewHolder> {
        ...
        public void onBindViewHolder(final ViewHolder holder, final int position) {
            holder.mTextView.setText(mValues.get(position).boardGameName);
